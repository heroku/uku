...
function initNewProject (name) {
  ...
  ncp(inputFullPath, outputShortPath, {
   ...
   const npmIgnoreFilePath = Path.join(outputFullPath, '/.npmignore');
   const gitIgnoreFilePath = Path.join(outputFullPath, '/.gitignore');
   try { require('fs').renameSync(npmIgnoreFilePath, gitIgnoreFilePath); } catch (er) { }
